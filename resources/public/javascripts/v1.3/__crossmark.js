...
var crossmark = {
	...
	initialize: function() {
		...
		this.sCrossMarkURL = this.sCrossMarkServer + '/dialog/?doi=' + this.sDOI + '&domain=' + this.sDomain + '&uri_scheme=' + this.sURIScheme + '&cm_version=' + this.scriptVersion();
		this.sStylesURL = this.sCDNServer + '/stylesheets/' + this.versionDir() + 'crossmark_widget.css';
	},
	...
	scriptVersion: function() {
		script = $cmjq('script[src$="crossmark.js"]')[0] || $cmjq('script[src$="crossmark.min.js"]')[0];
		var scriptName = script.src
		var parts = scriptName.split("/");
		var version = parts[parts.length - 2] == 'javascripts' ? 'v1.3' : parts[parts.length -2]
		return version;
